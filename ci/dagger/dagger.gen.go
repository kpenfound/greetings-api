...
import (
	...
	"github.com/kpenfound/greetings-api/ci/querybuilder"
)
...
// InfisicalGetSecretOpts contains options for Infisical.GetSecret
type InfisicalGetSecretOpts struct {
	Path string
}

// Get a secret from an Infisical project using secret name, project token, env, and path
func (r *Infisical) GetSecret(name string, token *Secret, projectId string, env string, opts ...InfisicalGetSecretOpts) *Secret {
	assertNotNil("token", token)
...
func (r *Infisical) UnmarshalJSON(bs []byte) error {
	...
	*r = *dag.LoadInfisicalFromID(InfisicalID(id))
	return nil
}
...
func (r *Client) LoadFlyFromID(id FlyID) *Fly {
	q := r.Query.Select("loadFlyFromID")
	q = q.Arg("id", id)
...
func (r *Client) LoadInfisicalFromID(id InfisicalID) *Infisical {
	q := r.Query.Select("loadInfisicalFromID")
	q = q.Arg("id", id)
...
func (r *Utils) Multisync(ctx context.Context, ctrs []*Container) ([]Container, error) {
	...
	convert := func(fields []multisync) []Container {
		out := []Container{}
