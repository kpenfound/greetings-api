...
import (
	...
	dagger "github.com/kpenfound/greetings-api/.dagger/internal/dagger/clients/code-workspace"
)
...
func CodeWorkspace(source *dagger.Directory, checker *dagger.CodeWorkspaceCheckable) *dagger.CodeWorkspace {
	client := initClient()
	return client.CodeWorkspace(source, checker)
}
...
func LoadCodeWorkspaceCheckableFromID(id dagger.CodeWorkspaceCheckableID) *dagger.CodeWorkspaceCheckable {
	client := initClient()
	return client.LoadCodeWorkspaceCheckableFromID(id)
}

// Load a CodeWorkspace from its ID.
func LoadCodeWorkspaceFromID(id dagger.CodeWorkspaceID) *dagger.CodeWorkspace {
	client := initClient()
	return client.LoadCodeWorkspaceFromID(id)
}
